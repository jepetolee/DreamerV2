...
class BasicEncoderBlock(nn.Module):
    def __init__(self, inputs, depth):
        super().__init__()
        self.depth = depth
        self.residual1 = nn.Sequential(nn.Conv2d(inputs, depth, 1, stride=2, bias=False), nn.BatchNorm2d(depth), nn.ELU(),
                                       nn.Conv2d(depth, depth, 1, stride=2, bias=False), nn.BatchNorm2d(depth), nn.ELU())
        self.conv1 = nn.Sequential(nn.BatchNorm2d(inputs), nn.ELU(), nn.Conv2d(inputs, depth, 3, stride=2))
        self.conv2 = nn.Sequential(nn.BatchNorm2d(depth), nn.ELU(), nn.Conv2d(depth, depth, 3, stride=2))
