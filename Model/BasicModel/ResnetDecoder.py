...
class BasicDecoderBlock(nn.Module):
    def __init__(self, inputs, depth):
        super().__init__()
        self.depth = depth
        self.residual1 = nn.Sequential(nn.ConvTranspose2d(inputs, depth, 1, stride=2, bias=False), nn.BatchNorm2d(depth), nn.ELU(),
                                       nn.ConvTranspose2d(depth, depth, 1, stride=2, bias=False), nn.BatchNorm2d(depth), nn.ELU())
        self.conv1 = nn.Sequential(nn.BatchNorm2d(inputs), nn.ELU(), nn.ConvTranspose2d(inputs, depth, 3, stride=2))
        self.conv2 = nn.Sequential(nn.BatchNorm2d(depth), nn.ELU(), nn.ConvTranspose2d(depth, depth, 3, stride=2))
    ...
    def forward(self, image):
        ...
        x = self.conv1(image)
        x = self.conv2(x)
        Trans = transforms.Resize(size=(x.shape[2],x.shape[3]))
        skip = Trans(skip)
        return skip + 0.1 * x
...
class ObservationResnetDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        AfterLinear = torch.reshape(AfterLinear,(-1, 256, 1, 5 ))
