...
from stag.Model.BasicModel import ObservationResnetEncoder, ObservationResnetDecoder
...
class TradingModel(nn.Module):
    def __init__(self, output_size, stochastic_size=30, deterministic_size=200,
        ...
        super().__init__()
        self.observation_encoder = ObservationResnetEncoder().to(device)

        encoder_embed_size = 1280
        embedding_size = 1280
        #need to check embedding size
        self.observation_decoder = ObservationResnetDecoder(stochastic_size, deterministic_size, embedding_size).to(device)
